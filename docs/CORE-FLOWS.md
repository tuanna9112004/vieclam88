...
**Quy ước diễn đạt phân quyền sau TASK 2.1:** "nhân sự thuộc đúng cơ sở hoặc Super Admin" nghĩa
là role thuộc `{staff, branch_admin}` VÀ `users.branch_id = owner_branch_id`, HOẶC role là
`super_admin` (không giới hạn cơ sở, `branch_id=null`). Các đoạn lịch sử bên dưới còn ghi
"Admin" được hiểu là **Super Admin**; TASK 2.3 sẽ harden toàn bộ ma trận query/export/dashboard.

## Định hướng Phase 2 (ADR-080, đang triển khai theo task)
...
- **Phân quyền (TASK 2.1 đã áp dụng)**: ba role `staff`/`branch_admin`/`super_admin`; dữ liệu
  `admin` cũ được backfill thành `super_admin`. Branch Admin quản lý Staff và cập nhật Branch
  trong cơ sở mình; TASK 2.3 tiếp tục audit/harden toàn bộ branch isolation.
...
### 0.1. `users` chỉ phục vụ ba role HR

Vì không có Candidate Account, bảng `users` chỉ có `staff`, `branch_admin`, `super_admin`. Không
