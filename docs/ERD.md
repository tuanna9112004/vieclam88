...
**`administrative_unit_mappings` (TASK 1.2):** bảng chuyển tiếp `administrative_units.id` →
`(province_id, ward_id)` mới, không thuộc kiến trúc target ở trên và không xuất hiện trong sơ đồ
— chi tiết cột ở `docs/DATABASE-DICTIONARY.md` mục "Bảng chuyển tiếp migration". Dự kiến drop ở
TASK 12.4 sau khi FK ward (TASK 1.3) đã switch ổn định.
